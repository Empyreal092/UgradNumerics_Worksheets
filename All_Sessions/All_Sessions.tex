...
\section{Some tips about plotting}
\subsection{}
...
\section{Interpolation and quadrature basics***}
\begin{itemize}
...
\item True or False? Gauss quadrature with 3 integration nodes
(and corresponding weights) is exact for polynomials of degree
7.
\item A Gauss quadrature rule with how many points is required to
integrated polynomials of degree 10 exactly?
\item  What is the result obtained with Simpson's rule for
integrating $f(x) = x^2$ over the interval $[0,1]$?
...
\section{Orthogonal polynomial and Gauss quadrature}
We assume
...
\subsection{***}
Use this family of orthogonal polynomials to compute the
    quadrature nodes of a 2-point Gauss quadrature rule.
    Compute the corresponding quadrature weights by using that
    constant and linear functions must be integrated exactly by this
    rule.
    
\subsection{***}
Up to what polynomial order is this quadrature rule exact for
    integrals over $[-1,1]$ weighted by $\omega$?

\section{More Gauss quadrature***}
Let $\omega:[-1,1]\to \mathbb
  R_{>0}$ be a weight function for the inner product $\langle
  f,g\rangle:=\int_{-1}^1\omega(x)f(x)g(x)\,dx$. Assume that $\omega$
  is continuous and strictly monotonically increasing. We want to
  compute a Gauss quadrature formula with a single node $x_0$. Show
  that $x_0>0$.
